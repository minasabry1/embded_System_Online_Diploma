...
E_STATUS_t TIMER0_Init(Timer_Config_t* configuartion){
	...
	if (NULL_PTR == configuartion){
		...
		if ((Global_configuartion->CLK >= NO_CLK_SRC_No_CTC) && (Global_configuartion->CLK <= EXT_CLK_RISING_No_CTC)){
			TCCR0 |= Global_configuartion->CLK;
		}
		else{
			Value = E_NOK;
		}
		if ((Global_configuartion->Compare == OCIE_DISABLE_No_CTC) || (Global_configuartion->Compare == OCIE_ENABLE_No_CTC)){
			TIMSK |= Global_configuartion->Compare;
		}
		else{
			Value = E_NOK;
		}
		if ((Global_configuartion->Overflow == TOIE_DISABLE_No_CTC) || (Global_configuartion->Overflow == TOIE_ENABLE_No_CTC)){
